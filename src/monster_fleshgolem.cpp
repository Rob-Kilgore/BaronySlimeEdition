/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_fleshgolem.cpp
	Desc: implements all of the fleshgolem monster's code
...
void initFleshGolem(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			if ( !strncmp(myStats->name, "enslaved fleshgolem", strlen("enslaved fleshgolem")) )
			{
			...
			else if ( !lesserMonster )
			{
				...
				for ( c = 0; c < 3; c++ )
				{
					Entity* entity = summonMonster(FLESH_GOLEM, my->x, my->y);
					if ( entity )
					{
	...
	// torso
	Entity* entity = newEntity(1577, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	...
	entity->focalx = limbs[FLESH_GOLEM][1][0]; // 0
	entity->focaly = limbs[FLESH_GOLEM][1][1]; // 0
	entity->focalz = limbs[FLESH_GOLEM][1][2]; // 0
	entity->behavior = &actFleshGolemLimb;
	entity->parent = my->getUID();
	...
	// right leg
	entity = newEntity(1581, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[FLESH_GOLEM][2][0]; // 1
	entity->focaly = limbs[FLESH_GOLEM][2][1]; // 0
	entity->focalz = limbs[FLESH_GOLEM][2][2]; // 2
	entity->behavior = &actFleshGolemLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left leg
	entity = newEntity(1580, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[FLESH_GOLEM][3][0]; // 1
	entity->focaly = limbs[FLESH_GOLEM][3][1]; // 0
	entity->focalz = limbs[FLESH_GOLEM][3][2]; // 2
	entity->behavior = &actFleshGolemLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right arm
	entity = newEntity(1579, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[FLESH_GOLEM][4][0]; // -.25
	entity->focaly = limbs[FLESH_GOLEM][4][1]; // 0
	entity->focalz = limbs[FLESH_GOLEM][4][2]; // 3
	entity->behavior = &actFleshGolemLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left arm
	entity = newEntity(1578, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[FLESH_GOLEM][5][0]; // -.25
	entity->focaly = limbs[FLESH_GOLEM][5][1]; // 0
	entity->focalz = limbs[FLESH_GOLEM][5][2]; // 3
	entity->behavior = &actFleshGolemLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
...
#define FLESH_GOLEMWALKSPEED .125
		...
		if ( bodypart == LIMB_HUMANOID_RIGHTLEG || bodypart == LIMB_HUMANOID_LEFTARM )
		{
			...
			else
			{
				if ( dist > 0.1 )
				{
					...
					{
						entity->pitch -= dist * FLESH_GOLEMWALKSPEED;
		...
		else if ( bodypart == LIMB_HUMANOID_LEFTLEG || bodypart == LIMB_HUMANOID_RIGHTARM )
		{
			...
			else
			{
				if ( dist > 0.1 )
				{
					if ( entity->skill[0] )
					{
						entity->pitch -= dist * FLESH_GOLEMWALKSPEED * .5;
