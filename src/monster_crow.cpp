/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_crow.cpp
	Desc: implements all of the crow monster's code
...
void initCrow(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			// always give special spell to crow, undroppable.
	...
	// torso
	Entity* entity = newEntity(1536, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->focaly = 1;
	...
	entity->focalx = limbs[CROW][1][0]; // 0
	entity->focaly = limbs[CROW][1][1]; // 1
	entity->focalz = limbs[CROW][1][2]; // 0
	entity->behavior = &actCrowLimb;
	...
	// right leg
	entity = newEntity(1538, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][2][0]; // 0
	entity->focaly = limbs[CROW][2][1]; // 0
	entity->focalz = limbs[CROW][2][2]; // 2
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left leg
	entity = newEntity(1537, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][3][0]; // 0
	entity->focaly = limbs[CROW][3][1]; // 0
	entity->focalz = limbs[CROW][3][2]; // 2
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][4][0]; // 0
	entity->focaly = limbs[CROW][4][1]; // 0
	entity->focalz = limbs[CROW][4][2]; // 3
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][5][0]; // 0
	entity->focaly = limbs[CROW][5][1]; // 0
	entity->focalz = limbs[CROW][5][2]; // 3
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	*/
	// right wing
	entity = newEntity(1540, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][6][0]; // 0
	entity->focaly = limbs[CROW][6][1]; // 4
	entity->focalz = limbs[CROW][6][2]; // 0
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left wing
	entity = newEntity(1539, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[CROW][7][0]; // 0
	entity->focaly = limbs[CROW][7][1]; // -4
	entity->focalz = limbs[CROW][7][2]; // 0
	entity->behavior = &actCrowLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	for (bodypart = 0, node = my->children.first; node != nullptr; node = node->next, bodypart++)
	{
		...
		if ( bodypart == 3 || bodypart == 6 )
		{
			...
			if ( bodypart == 3 || !MONSTER_ATTACK )
			{
				...
				if ( !leftbody->skill[0] )
				{
					entity->pitch -= CROWWALKSPEED * limbSpeedMultiplier;
			...
			else
			{
				...
				else if ( MONSTER_ATTACK == MONSTER_POSE_MELEE_WINDUP3 )
				{
					if ( MONSTER_ATTACKTIME == 0 )
					{
						...
						if ( multiplayer != CLIENT )
						{
							// crow can't be paralyzed, use EFF_STUNNED instead.
						...
						case 2:
							...
							if ( limbAnimateToLimit(entity, ANIMATE_PITCH, -0.4, 5 * PI / 4, false, 0) == 1 )
							{
								...
								if ( multiplayer != CLIENT )
								{
									...
									my->attack(MONSTER_POSE_CROW_DOUBLEATTACK, 0, nullptr);
								}
							}
							break;
						default:
				...
				// double attack
				else if ( MONSTER_ATTACK == MONSTER_POSE_CROW_DOUBLEATTACK )
				{
					switch ( entity->skill[0] )
					{
						case 2:
