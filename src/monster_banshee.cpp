/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_banshee.cpp
	Desc: implements all of the banshee monster's code
...
void initBanshee(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			// always give special spell to banshee, undroppable.
	...
	// torso
	Entity* entity = newEntity(1526, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->focaly = 1;
	...
	entity->focalx = limbs[BANSHEE][1][0]; // 0
	entity->focaly = limbs[BANSHEE][1][1]; // 1
	entity->focalz = limbs[BANSHEE][1][2]; // 0
	entity->behavior = &actBansheeLimb;
	...
	// right leg
	entity = newEntity(1528, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][2][0]; // 0
	entity->focaly = limbs[BANSHEE][2][1]; // 0
	entity->focalz = limbs[BANSHEE][2][2]; // 2
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left leg
	entity = newEntity(1527, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][3][0]; // 0
	entity->focaly = limbs[BANSHEE][3][1]; // 0
	entity->focalz = limbs[BANSHEE][3][2]; // 2
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right arm
	entity = newEntity(1530, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][4][0]; // 0
	entity->focaly = limbs[BANSHEE][4][1]; // 0
	entity->focalz = limbs[BANSHEE][4][2]; // 3
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left arm
	entity = newEntity(1529, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][5][0]; // 0
	entity->focaly = limbs[BANSHEE][5][1]; // 0
	entity->focalz = limbs[BANSHEE][5][2]; // 3
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right wing
	entity = newEntity(1532, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][6][0]; // 0
	entity->focaly = limbs[BANSHEE][6][1]; // 4
	entity->focalz = limbs[BANSHEE][6][2]; // 0
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left wing
	entity = newEntity(1531, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BANSHEE][7][0]; // 0
	entity->focaly = limbs[BANSHEE][7][1]; // -4
	entity->focalz = limbs[BANSHEE][7][2]; // 0
	entity->behavior = &actBansheeLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	for (bodypart = 0, node = my->children.first; node != nullptr; node = node->next, bodypart++)
	{
		...
		if ( bodypart == 3 || bodypart == 6 )
		{
			...
			if ( bodypart == 3 || !MONSTER_ATTACK )
			{
				...
				if ( !leftbody->skill[0] )
				{
					entity->pitch -= BANSHEEWALKSPEED * limbSpeedMultiplier;
			...
			else
			{
				...
				else if ( MONSTER_ATTACK == MONSTER_POSE_MELEE_WINDUP3 )
				{
					if ( MONSTER_ATTACKTIME == 0 )
					{
						...
						if ( multiplayer != CLIENT )
						{
							// banshee can't be paralyzed, use EFF_STUNNED instead.
						...
						case 2:
							...
							if ( limbAnimateToLimit(entity, ANIMATE_PITCH, -0.4, 5 * PI / 4, false, 0) == 1 )
							{
								...
								if ( multiplayer != CLIENT )
								{
									...
									my->attack(MONSTER_POSE_BANSHEE_DOUBLEATTACK, 0, nullptr);
								}
							}
							break;
						default:
				...
				// double attack
				else if ( MONSTER_ATTACK == MONSTER_POSE_BANSHEE_DOUBLEATTACK )
				{
					switch ( entity->skill[0] )
					{
						case 2:
