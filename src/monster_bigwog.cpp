/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_bigwog.cpp
	Desc: implements all of the bigwog monster's code
...
void initBigWog(Entity* my, Stat* myStats)
{
	...
	if (multiplayer != CLIENT && !MONSTER_INIT)
	{
		if (myStats != NULL)
		{
			...
			{
				strcpy(myStats->name, "Revenant BigWog");
				myStats->HP += 500;
				myStats->MAXHP += 500;
				myStats->STR = 65;
				myStats->DEX = 30;
			...
			// random effects
			// bigwogs can traverse waters and pits (pits with magic :))
	...
	// head
	Entity* entity = newEntity(1507, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	...
	entity->focalx = limbs[BIGWOG][1][0]; // 0
	entity->focaly = limbs[BIGWOG][1][1]; // 0
	entity->focalz = limbs[BIGWOG][1][2]; // 0
	entity->behavior = &actBigWogLimb;
	...
	// chest
	entity = newEntity(1508, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BIGWOG][2][0]; // 0
	entity->focaly = limbs[BIGWOG][2][1]; // 0
	entity->focalz = limbs[BIGWOG][2][2]; // 0
	entity->behavior = &actBigWogLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right leg
	entity = newEntity(1512, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BIGWOG][3][0]; // 1
	entity->focaly = limbs[BIGWOG][3][1]; // 0
	entity->focalz = limbs[BIGWOG][3][2]; // 5
	entity->behavior = &actBigWogLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left leg
	entity = newEntity(1511, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BIGWOG][4][0]; // 1
	entity->focaly = limbs[BIGWOG][4][1]; // 0
	entity->focalz = limbs[BIGWOG][4][2]; // 5
	entity->behavior = &actBigWogLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// right arm
	entity = newEntity(1510, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BIGWOG][5][0]; // 2.5
	entity->focaly = limbs[BIGWOG][5][1]; // 7
	entity->focalz = limbs[BIGWOG][5][2]; // 3.5
	entity->behavior = &actBigWogLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	// left arm
	entity = newEntity(1509, 0, map.entities, nullptr); //Limb entity.
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	entity->flags[NOUPDATE] = true;
	entity->flags[USERFLAG2] = my->flags[USERFLAG2];
	entity->focalx = limbs[BIGWOG][6][0]; // 2.5
	entity->focaly = limbs[BIGWOG][6][1]; // -7
	entity->focalz = limbs[BIGWOG][6][2]; // 3.5
	entity->behavior = &actBigWogLimb;
	entity->parent = my->getUID();
	node = list_AddNodeLast(&my->children);
	node->element = entity;
	...
	for (bodypart = 0, node = my->children.first; node != NULL; node = node->next, bodypart++)
	{
		...
		if (bodypart == 4 || bodypart == 7)
		{
			...
			if (dist > 0.1)
			{
				...
				{
					entity->pitch -= dist * BIGWOGWALKSPEED;
					if (entity->pitch < -PI / 6.0)
					{
						entity->pitch = -PI / 6.0;
...
#define BIGWOGTRAP_FIRED my->skill[0]

void actBigWogTrap(Entity* my)
{
	if (!my->skill[28])
	{
		return;
	...
	if (my->skill[28] == 2)
	{
		if (!BIGWOGTRAP_FIRED)
		{
			Entity* monster = summonMonster(BIGWOG, my->x, my->y);
			if (monster)
			{
				BIGWOGTRAP_FIRED = 1;
...
#define BIGWOGTIMER_LIFE my->skill[0]
#define BIGWOGTIMER_ACTIVE my->skill[1]

void actBigWogTimer(Entity* my)
{
	node_t* node;

	BIGWOGTIMER_LIFE++;
	if (((currentlevel < 45 && BIGWOGTIMER_LIFE == TICKS_PER_SECOND * 120)
		|| (currentlevel >= 45 && BIGWOGTIMER_LIFE == TICKS_PER_SECOND * 180)
		)
	...
	else if (((currentlevel < 45 && BIGWOGTIMER_LIFE >= TICKS_PER_SECOND * 150)
		|| (currentlevel >= 45 && BIGWOGTIMER_LIFE >= TICKS_PER_SECOND * 210)
		)
		&& !BIGWOGTIMER_ACTIVE)     // two and a half minutes if currentlevel < 45, else 3.5 minutes
	{
		Entity* monster = summonMonster(BIGWOG, my->x, my->y);
		if (monster)
		{
			...
			BIGWOGTIMER_ACTIVE = BIGWOGTIMER_LIFE;
		}
	}
	if (BIGWOGTIMER_ACTIVE && BIGWOGTIMER_LIFE >= BIGWOGTIMER_ACTIVE + TICKS_PER_SECOND * 3)
	{
		int c;
		for (c = 0; c < MAXPLAYERS; c++)
		{
...
void actBigWogCeilingBuster(Entity* my)
{
	double x, y;
			...
			if (x >= 0 && y >= 0 && x < map.width << 4 && y < map.height << 4)
			{
				...
				if (map.tiles[index])
				{
					...
					if (multiplayer != CLIENT)
					{
						...
						if (myStats)
						{
							// easy hack to stop the bigwog while he breaks stuff
							myStats->EFFECTS[EFF_PARALYZED] = true;
...
void createBigWogTimer(Entity* entity, map_t* map)
{
	...
	childEntity->behavior = &actBigWogTimer;
