/*-------------------------------------------------------------------------------
	...
	BARONY
	File: monster_jackolantern.cpp
	Desc: implements all of the jackolantern monster's code
...
void initJackOLantern(Entity* my, Stat* myStats)
{
		...
		if ( myStats != NULL )
		{
			...
			if ( rand() % 50 == 0 && !my->flags[USERFLAG2] && !myStats->MISC_FLAGS[STAT_FLAG_DISABLE_MINIBOSS] )
			{
				...
				case 0:
					...
					for (c = 0; c < 3; c++)
					{
						Entity* entity = summonMonster(JACK_O_LANTERN, my->x, my->y);
						if (entity)
						{
	...
	for ( c = 0; c < 4; c++ )
	{
		...
		// "shin"
		entity = newEntity(1548 + c, 0, map.entities, nullptr); //Limb entity.
		entity->sizex = 4;
		entity->sizey = 4;
		...
		entity->focalx = limbs[JACK_O_LANTERN][4][0]; // 3
		entity->focaly = limbs[JACK_O_LANTERN][4][1]; // 0
		entity->focalz = limbs[JACK_O_LANTERN][4][2]; // 0
		entity->behavior = &actJackOLanternLimb;
		entity->parent = my->getUID();
